...
public class Invalidate {
	...
	@Produces(MediaType.APPLICATION_JSON)
	public String invalidate(@Context HttpServletRequest requestContext) {
		...
		try {
			...
			if (!result.getString("clientToken").equals(DigestUtils.sha512Hex(request.get("clientToken").getAsString()))) {
				result = null;
				request = null;
			...
			preparedStatement = connection.prepareStatement("DELETE FROM `clients` WHERE `clientId`=? LIMIT 0 , 1");
			preparedStatement.setString(1, request.get("clientId").getAsString());
